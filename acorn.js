...
function setOptions(opts) {
    options = opts || {};
    for (var opt in exports.defaultOptions)
        if (!has(options, opt))
            options[opt] = exports.defaultOptions[opt];
...
function raise(pos, message) {
    var loc = exports.getLineInfo(input, pos);
...
function readToken(forceRegexp) {
    ...
    if (!inTemplate && (exports.isIdentifierStart(code) || code === 92))
        return readWord();
...
function readRadixNumber(radix) {
    ...
    if (exports.isIdentifierStart(input.charCodeAt(tokPos)))
...
function readWord1() {
    ...
    for (; ;) {
        var ch = input.charCodeAt(tokPos);
        if (exports.isIdentifierChar(ch)) {
            if (containsEsc)
            ...
            if (!(first ? exports.isIdentifierStart(esc) : exports.isIdentifierChar(esc)))
...
function parseForStatement(node) {
    ...
    if (tokType === _var || tokType === _let) {
        ...
        if ((tokType === _in || (tokType === _name && tokVal === "of")) && init.declarations.length === 1 && !(isLet && init.declarations[0].init))
